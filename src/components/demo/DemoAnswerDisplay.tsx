          ...
          {isKakapo && (
            <div className="flex items-center gap-1 border border-border rounded overflow-hidden">
              <button
                ...
                className={`text-2xs font-mono px-2.5 py-1 transition-colors cursor-pointer border-0 ${businessMode ? "bg-accent text-white" : "bg-transparent text-text-muted hover:text-text-primary"}`}
              >
                Business
              </button>
              <button
      ...
      {isKakapo && result.cited_kpts.length > 0 && (
        <div>
          <p className="text-2xs font-mono text-accent uppercase tracking-widest mb-3">
            {result.cited_kpts.length} source{result.cited_kpts.length > 1 ? "s" : ""} {businessMode ? "vérifiées" : "KAKAPO"}
          </p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {result.cited_kpts.map((kpt, i) => (
              <CitedKPTCard key={kpt.kpt_id} kpt={kpt} index={i + 1} businessMode={businessMode} />
            ))}
          </div>
          <div className="mt-4">
